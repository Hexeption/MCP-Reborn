...
5. (OPTIONAL) Run the Gradle "copyInbuildAssetsAsResources" task to copy the embedded assets from the base version's JAR file and add them as resources to the project(requires having the base version in the default installation path). This will avoid the need to manually copy using an archive manager
...
7. With that JAR generated, open your Minecraft versions folder. On Linux, this defaults to `~/.minecraft/versions`. On
...
11. (If you have not done point 5) Do one of the following:
- Using an archive manager (Ubuntu comes with one built in; Windows users can download 7-Zip), open the base version's
...
- Run the Gradle" copyInbuildAssetsToJar" task to copy the embedded resources from the base version JAR file and add them to the new file(requires having the base version in the default installation path).
